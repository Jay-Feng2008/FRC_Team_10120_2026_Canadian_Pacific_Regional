...
public class FlywheelIOReal implements FlywheelIO {
  ...
  public FlywheelIOReal(int id, boolean isclockwice_Positive) {
    ...
    talon = new TalonFX(FlywheelConstants.flywheelId);
    talonR = new TalonFX(1);
    ...
    final TalonFXConfiguration config =
        new TalonFXConfiguration()
            ...
            .withFeedback(
                new FeedbackConfigs()
                    .withSensorToMechanismRatio(FlywheelConstants.flywheelGearRatio))
            .withCurrentLimits(
    ...
    tryUntilOk(5, () -> talonR.getConfigurator().apply(config));
    talonR.setControl(new Follower(talon.getDeviceID(), MotorAlignmentValue.Opposed));
    // talonR.setControl(new Follower(talon.getDeviceID(), false));
    // 初始化信号 跟随电机只需要读取applyvoltage
    ...
    followerAppliedVoltage = talonR.getMotorVoltage();
